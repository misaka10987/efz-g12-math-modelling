  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box, fill=mainblue!12] (earth) {基地生产力 $B(t)$};
...
\begin{frame}{问题抽象与基本假设}
  \begin{block}{核心抽象：离散时间最优控制问题}
    ...
    每一代需决策总生产力在"基地再投资"与"殖民投入"之间的分配比例。
  \end{block}
  \vspace{0.3cm}
  \textbf{关键假设：}
  \begin{enumerate}
  ...
  \note{
    ...
    想象你有两颗星球：基地和殖民地。每一代，你需要决定把多少生产力用于基地自身发展，
...
\begin{frame}{数学模型构建}
  ...
  \note{
    这是模型的核心方程。公式1描述了基地的更新：每代基地新增加的生产力
    等于增长率乘以投入量再乘以Logistic因子。Logistic因子的含义是：
    当基地越接近其环境容量上限，增长的边际效益越低。
...
\begin{frame}{参数体系}
  \begin{table}
    ...
    \begin{tabular}{llcl}
      ...
      \midrule
      基地生产力 & $B(t)$ & 状态变量 & 基地在第 $t$ 代的生产力\\
      殖民地生产力 & $C(t)$ & 状态变量 & 殖民地在第 $t$ 代的生产力\\
      基地投资 & $I_{\text{base}}(t)$ & 决策变量 & 第 $t$ 代投入基地再发展的生产力\\
      殖民地投资 & $I_{\text{colony}}(t)$ & 决策变量 & 第 $t$ 代用于殖民建设的投入\\
      模拟代数       & $T$               & 120    & 模拟的总世代数 \\
      增长率         & $g$               & $2^{1/12}-1$ & 每代增长率（12代翻倍）\\
      殖民成本系数   & $\text{cost}$     & 5      & 殖民投入的效率折扣 \\
      殖民延迟       & $d$               & 1      & 投入生效所需世代数 \\
      基地容量上限   & $K_{\text{base}}$ & 10     & 基地Logistic增长上限 \\
...
\begin{frame}{策略总览}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[l1g] (cat1) {基线策略};
      \node[l1b, right=1.5cm of cat1] (cat2) {启发式策略};
      \node[l1d, right=1.5cm of cat2] (cat3) {优化策略};
  ...
  \note{
    ...
    永不殖民是纯基地发展的参考基准；固定比例则按恒定k值分配资源。

    中间层是三个启发式策略：分段切换、硬贪心和软贪心。
    分段策略在基地利用率达到阈值后切换殖民比例；
...
\begin{frame}{基线策略与启发式策略}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item \textbf{分段切换}：
        \begin{itemize}
          \item $B(t)/K_{\text{base}} < \theta$：全投基地
  ...
  \note{
    ...
    100%投入基地。这个策略会收敛到基地的Logistic上限，
    是一个自然的参考基准——如果任何殖民策略连这个都打不过，
    那殖民就没有意义了。固定比例策略则以恒定的k值分配，
    简单直观，但问题是它无法适应动态变化的环境。

    启发式策略更聪明一些。分段切换策略会监控基地的利用率，
    当利用率低于阈值时全投基地，超过阈值后才开始殖民。
    这模拟了一种"先发展后扩张"的思路。
...
\begin{frame}{策略对比分析}
  ...
  \note{
    ...
    从预期表现来看，永不殖民策略会收敛到基地的Logistic上限，
...
\begin{frame}{关键发现}
  ...
  \note{
    ...
    第一，Logistic饱和是殖民的根本动因。如果基地可以无限增长，
    那殖民就没有意义——把钱留在基地复利就好了。
    正是因为基地有容量上限，当接近上限时边际回报急剧下降，
    殖民地才提供了一条全新的增长曲线。
